...
def process_video(input_video_path, output_video_path, seg_model_path, detection_model_path, score_threshold=0.3, orientation='left', background_color='black'):
    ...
    if not cap.isOpened():
        print(f"Error: Unable to open video from {input_video_path}")
        return
    ...
    while True:
        ...
        # Isolate the person in the current frame
        person_isolated_frame = isolate_person_in_frame(frame, seg_model, score_threshold, background_color=background_color)
        ...
        for result in detection_results:
            ...
            for box in boxes:
                ...
                # Apply Gaussian smoothing with a larger kernel size
                ...
                not(orientation == 'left' and neck_point[0] >= back_point[0]) and not (orientation == 'right' and neck_point[0] <= back_point[0]):
                    ...
                    # Print the calculated angle and points to the terminal
                    # print(f"Calculated angle: {round(angle, 2)} degrees")
                    # print(f"Neck point: {neck_point}, Back point: {back_point}")
        ...
        out.write(person_isolated_frame)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Isolate person in video using YOLOv8 and process with another YOLOv8 model")
    ...
    process_video(args.input, args.output, args.seg_model, args.detection_model, args.score_threshold, args.orientation, args.background_color)